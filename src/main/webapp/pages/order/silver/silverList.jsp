...
<script type="text/javascript">
	...
	$("#menu-4-1,#menu-4-1 a").addClass("active");
	$(function(){
		$(".select2").select2();
		if ($("#bdId").length == 0) {
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
     table=$("#datatable").DataTable({
           ...
           dom: 'T<"clear">lfrtip',
           ajax:{
               "url":"<%=request.getContextPath()%>/order/silverdata",
               "data":function(d){
           ...
           columnDefs:[
				...
				},
				{
			     ...
			     render: function (data, type, row, meta) {
			  	   var returnString ="";
			  	   if(row.status==0){
			      	   returnString +=  '<a type="button" class="btn btn-success" onclick=updateLogisticsNumberPage(' + row.id + ',1) >发货</a>'+
			             	'<a type="button" class="btn btn-danger "  onclick=updateStatus(' + row.id + ',2) >取消</a>';
			         }else if(row.status==1){
			      	   returnString +=  '<a type="button" class="btn btn-primary" onclick=getLogisticsNumber(' + row.id + ') >单号</a>'+
			      	   	 	'<a type="button" class="btn btn-danger "  onclick=updateStatus(' + row.id + ',4) >退货</a>'+
			      	   		'<a type="button" class="btn btn-success "  onclick=updateStatus(' + row.id + ',3) >完成</a>';
			         }
			         return returnString;
			     }
				},
    ...
    function updateStatus(id,status){
        var url = "<%=request.getContextPath()%>/order/silverupdateStatus";
