...
//     $("#status").removeClass("select2-hidden-accessible");
    ...
    $(document).ready(function() {
     table=$("#datatable").DataTable({
           ...
           dom: 'T<"clear">lfrtip',
           ajax:{
               "url":"<%=request.getContextPath()%>/publisher/publisher!publisherList",
               "data":function(d){
                   d.id=$("#id").val();
                   d.name=$("#name").val();
                   d.email=$("#email").val();
           ...
           columnDefs:[
				{
				    ...
				    render: function (data, type, row, meta) {
				        ...
				        if(row.status==0){
				            returnString +=  '<a type="button" class="btn btn-success"  href="#" onclick=updatePublisherStatus(' + row.id + ',1) >Approve</a>'+
				            '<a type="button" class="btn btn-danger "  href="#" onclick=updatePublisherStatus(' + row.id + ',2) >Deny </a>';
				        }else if(row.status==1){
				            returnString +=   '<a type="button" class="btn btn-danger"  href="#" onclick=updatePublisherStatus(' + row.id + ',3) >Block </a>';
    ...
    function updatePublisherVO(id){
    	post('<%=request.getContextPath()%>/publisher/publisher!publisherUpdatePage', {"publisherVO.id":id});
    }
    
    function updatePublisherStatus(id,status){
        var url = "<%=request.getContextPath()%>/publisher/publisher!publisherUpdateStatus";
        $.post(url,{"publisherVO.id":id,"publisherVO.status":status},function(rs){
        	if(rs == 1){
        		showMiddle("Success");
        	}else{
        		showMiddle("Fail");
